...
They asked us for a program that tells us how many seconds it takes for all the leds to turn on. The seconds are expressed as an integer. For 
example:
...
function countTime(leds: number[]): number {
  ...
  let seconds = 0;
  let init =
    leds.findIndex((led) => led === 1) === leds.length - 1
      ? 0
      : leds.findIndex((led) => led === 1) + 1;

  while (init <= leds.length - 1 && leds.some((led) => led === 0)) {
    if (leds[init] === 0) {
      if (init !== 0 || leds[leds.length - 1] === 1) seconds += TIME_CHANGE;
      leds[init] = 1;
    }
    if (init === leds.length - 1) {
      init = 0;
      continue;
    }
    init++;
